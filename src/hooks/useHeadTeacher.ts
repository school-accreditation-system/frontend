  ...
  return useMutation<string, Error, HeadTeacherDTO>({
    mutationFn: async (data: HeadTeacherDTO) => {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/qamis/school/addheadteacher`,
        data
      );
  ...
  >({
    ...
    queryFn: async () => {
      const response = await axios.get<{
        ...
        `${process.env.NEXT_PUBLIC_API_URL}/qamis/school/head-teacher/get-head-teacher?schoolId=${schoolId}`
      );
      return response.data;
    },
  });
