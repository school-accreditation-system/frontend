...
const CombinationManagementPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCombinations = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/qamis/combination/getCombinationsBylevel?combinationId=${selectedLevel}`,
          {
            headers: {
              "qamis-request-key": "1234567890",
            },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/qamis/combination/save-combination?parentId=${selectedLevel}`,
        {
          method: "POST",
          headers: {
