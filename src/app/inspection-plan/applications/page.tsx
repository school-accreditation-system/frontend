...
import Pagination from
  "@/components/inspection-plan/_components/Pagination";
...
const RequestDirectoryPage = () => {
    ...
    async function fetchRequestData() {
      ...
      try {
        const response = await
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/qamis/request/getAllRequests`);
        if (!response.ok) {
  ...
  const uniqueStatuses = useMemo(() => {
    ...
    return ["All", ...new Set(requestData.map((request) =>
      request.status).filter(Boolean))];
  }, [requestData]);
  // Sorting function
      ...
      .filter(
        (request) =>
          ...
          (request.email?.toLowerCase().includes(searchTerm.toLowerCase()
            ...
            request.nationalId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            request.phoneNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            request.applicantRole?.toLowerCase().includes(searchTerm.toLowerCase())) &&
          // Status filter
  ...
  return (
    <div className="p-6 mx-auto bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-bold mb-4 text-primary">Application
        Requests</h1>
...
focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div className="mb-4">
        ...
        <select
          id="pageSize"
          className="p-1 border border-gray-300 rounded focus:outline-
...
text-sm flex items-center">
        ...
        {searchTerm && (
          <div className="bg-blue-100 text-blue-500 px-3 py-1 rounded-full 
text-sm flex items-center">
            Search: "{searchTerm}"
            <button
        ...
        <table className="min-w-full bg-white">
          <thead className="bg-primary text-white">
            <tr>
              ...
              >
                National ID{" "}
                {sortField === "nationalId" && (sortDirection === "asc" ? "↑" :
                  "↓")}
              </th>
              <th
                className="py-3 px-4 text-left cursor-pointer"
                onClick={() => handleSort("applicantRole")}
              >
                Role{" "}
                {sortField === "applicantRole" && (sortDirection === "asc" ? "↑"
                  : "↓")}
              </th>
              <th
                className="py-3 px-4 text-left cursor-pointer"
...
{getStatusBadgeClass(request.status)} rounded-full text-sm`}>
                    {request.status}
                  </span>
                </td>
                <td className="py-3 px-4 border-b">
...
gray-700">Applicant Information</h3>
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{selectedRequest.nationalId}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Phone Number</p>
                      <p className="font-medium">{selectedRequest.phoneNumber}
                      </p>
                    </div>
                    <div>
...
rounded-full text-sm">
                    ...
                    <div>
                      ...
                      <p>
                        <span className={`px-2 py-1 $
{getStatusBadgeClass(selectedRequest.status)} rounded-full text-
sm`}>
                    ...
                    <div>
                      ...
                      <p className="font-
medium">{formatDate(selectedRequest.createdAt)}</p>
                    </div>
                  </div>
                </div>
                <div>
                  <h3 className="text-lg font-semibold mb-4 text-
gray-700">Assessment Information</h3>
                  {selectedRequest.assessment ? (
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm text-gray-500">Assessment ID</p>
                        <p className="font-medium">{selectedRequest.assessment.id}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">School</p>
                        <p className="font-
medium">{selectedRequest.assessment.school?.name || "N/A"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Combination</p>
                        <p className="font-
medium">{selectedRequest.assessment.combination?.name ||
                            "N/A"}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Ranking</p>
                        <p className="font-
medium">{selectedRequest.assessment.ranking || "Not ranked"}
                        </p>
                      </div>
                      <div>
                        ...
                        {selectedRequest.assessment.options &&
                          ...
                          <ul className="list-disc pl-5">
                            {selectedRequest.assessment.options.map((option, index) => (
                    ...
                    <p className="text-gray-500">No assessment information
                      available</p>
                  )}
                </div>
              </div>
...
gray-700">Documents</h3>
                  <div className="p-3 bg-gray-50 rounded border border-
gray-200">
                    <p className="flex items-center">
                      <svg className="w-5 h-5 mr-2 text-gray-500" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round"
                          strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 
