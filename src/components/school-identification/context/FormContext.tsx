...
type FormContextType = {
  ...
  updateFormData: (stepId: string, data: Partial<SchoolIdentificationFormValues>) => void;
...
export const FormProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [formData, setFormData] = useState<Partial<SchoolIdentificationFormValues>>({});
  ...
  const validateStep = async (stepId: string, schema?: any): Promise<boolean> => {
  ...
  const handleSubmit = async () => {
    ...
    for (const step of IDENTIFICATION_STEPS.slice(0, -1)) {
      ...
      const validationSchema = 'validationSchema' in step ? step.validationSchema : undefined;
