...
import { useGetCriteriaByAreaId, useGetIndicatorsByCriteriaId } from '@/hooks/useStandard';
import { zodResolver } from '@hookform/resolvers/zod';
import { AlertCircle, Building, CheckCircle2, ChevronRight, ClipboardList, Loader2 } from 'lucide-react';
import { useCallback, useEffect, useRef, useState } from 'react';
import { useForm } from 'react-hook-form';
import { FileUpload } from '../FileUpload';
import { createDynamicLandOwnershipSchema, landOwnershipSchema } from '../types/schema';
...
export const LandOwnershipForm = ({
    ...
    const { data: criteria, isLoading: criteriaLoading } = useGetCriteriaByAreaId(areaId);
            ...
            if (initialLoadRef.current) {
                ...
                Object.entries(parentFormData).forEach(([key, value]) => {
                    if (key.endsWith('_document') && value) {
                        ...
                        uploadedFilesRef.current.add(`${key}-${(value as File)?.name || 'file'}`);
                    }
                });
    ...
    const safeUpdateFormData = useCallback((data: any) => {
    ...
    // Handle file upload
    const handleFileUpload = useCallback((indicatorId: string, file: File | null) => {
    ...
    const validateCurrentCriteria = async () => {
        ...
        const missingIndicators = currentCriteriaIndicatorIds.filter(id => !formValues[id]);
        ...
        if (missingIndicators.length > 0) {
            ...
            if (firstMissingIndex !== -1) {
                ...
                toast({
                    ...
                    description: "Please answer all questions in this criteria before proceeding",
                    status: "error"
                });
        ...
        for (let i = 0; i < indicators.length; i++) {
            ...
            if (selectedValue) {
                ...
                const validOptions = indicator.standardOption?.map(opt => opt.id) || [];
        ...
        if (invalidSelection && invalidIndicatorIndex >= 0) {
            ...
            toast({
                ...
                description: "One of your selections is not valid for its question. Please select a valid option.",
                status: "error"
            });
            return false;
        }
        ...
        if (!allFieldsValid) {
            ...
            toast({
                ...
                description: "Some questions have invalid answers. Please review and correct them.",
                status: "error"
            });
            return false;
        }
    ...
    const handleNextButtonClick = async () => {
        ...
        const validOptions = currentIndicatorItem.standardOption?.map(opt => opt.id) || [];
        ...
        if (!currentFieldValid) {
            ...
            toast({
                ...
                description: "Please select an option for this question before proceeding",
                status: "error"
            });
            return;
        }
                    ...
                    for (const criterion of criteria) {
                        ...
                        const criteriaIndicators = await (async () => {
                            try {
                                ...
                                console.error(`Failed to fetch indicators for criteria ${criterion.id}:`, error);
                                return [];
                            }
                        })();

                        if (!criteriaIndicators || criteriaIndicators.length === 0) continue;

                        const criteriaIndicatorIds = criteriaIndicators.map(ind => ind.id);

                        // Check if all indicators in this criteria have values
                        const values = form.getValues();
                        const missingFields = criteriaIndicatorIds.filter(id => !values[id]);
                ...
                if (allValid) {
                    ...
                    if (zodValid) {
                        ...
                        toast({
                            ...
                            description: "There are validation errors in the form. Please check all fields.",
                            status: "error"
                        });
                    }
    ...
    const handleCriteriaChange = async (criterionName: string, criterionId: string) => {
    ...
    const renderIndicators = () => {
        ...
        const isCurrentValueValid = !currentValue || validOptionIds.includes(currentValue);
                            ...
                            <FormControl>
                                ...
                                >
                                    {indicatorOptions.map((option) => (
                                        <div key={option.id} className="flex hover:cursor-pointer hover:text-primary items-center space-x-2">
    ...
    if (!criteriaLoading && (!criteria || criteria.length === 0)) {
        return (
            <div className="flex flex-col justify-center items-center h-64">
                <p className="text-gray-500 mb-4">No indicators available for this criteria</p>
                <Button
                    onClick={async () => {
                        updateFormData({});
                        await onNext();
                        ...
                        <div className="space-y-3 p-3 md:p-4 bg-blue-50 rounded-lg">
                            {criteria && criteria.map((criterion) => (
                                <div
                                    ...
                                    onClick={() => handleCriteriaChange(criterion.name, criterion.id)}
                                >
                                ...
                                >
                                    {isLastCriteriaGroup() && currentIndicator === getIndicatorCountForCriteria() - 1
                                        ...
                                        : currentIndicator < getIndicatorCountForCriteria() - 1
                                            ? 'Next Question'
                                            : 'Next Criteria'} <ChevronRight className="h-3.5 w-3.5 md:h-4 md:w-4" />
                                </Button>
                            </div>
                        </div>
                    </div>
