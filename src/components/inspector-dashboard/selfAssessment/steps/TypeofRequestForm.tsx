...
export const TypeOfRequestForm = ({
  ...
  useEffect(() => {
    if (combinations && combinations.length > 0) {
      ...
      const uniqueCategories = Array.from(
        new Set(combinations.map((item) => item?.category))
      );
  ...
  const getCategoryItemCount = (category) => {
    return (
      combinations?.filter((item) => item?.category === category).length || 0
    );
  };
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
                    ...
                    {categories.map((category) => (
                      <div
                        ...
                        className={`p-3 rounded-lg cursor-pointer border transition-all duration-200 
                                                min-w-[150px] group ${getCategoryColor(
                                                  category
                                                )}`}
                      >
              ...
              <p className="text-sm text-gray-500">
                Select one of the following items and click next to continue
              </p>
              <FormField
                            ...
                            <div className="flex items-center mb-2">
                              ...
                              <div className="ml-3 flex flex-col">
                                <div className="flex items-center gap-2">
                                  <h3 className="font-medium mt-1 text-lg">
                                    {item?.fullName}
                                  </h3>
