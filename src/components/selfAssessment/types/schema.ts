...
export const typeOfRequestSchema = z.object({
  ...
  selectedCombination: z.string({
    required_error: "You must select a combination",
  }),
});

// Labels mapping for the type of request
export const typeOfRequestLabels: Record<
  z.infer<typeof typeOfRequestSchema>["typeOfRequest"],
  string
> = {
...
// Dynamic schema that can be updated based on API data
export const createDynamicLandOwnershipSchema = (
  indicatorIds: string[] = []
) => {
  // Create a dynamic schema object based on the indicator IDs
  const schemaFields: Record<string, z.ZodType<any>> = {};

  // Add a dynamic validation for each indicator
  for (const id of indicatorIds) {
    schemaFields[id] = z.string({
      required_error: `Selection is required for this indicator`,
    });
  }

  // If no indicators are provided, use the old schema structure
  if (indicatorIds.length === 0) {
    return landOwnershipSchema;
  }
...
export const landOwnershipSchema = z.object({
  strategicPlan: z.enum(
    ["no_plan", "basic_plan", "with_vision", "with_structure", "complete_plan"],
    {
      required_error: "Strategic plan selection is required",
    }
  ),
  annualActionPlan: z.enum(
    ["no_action_plan", "has_action_plan", "complete_action_plan"],
    {
      required_error: "Annual action plan selection is required",
    }
  ),
  annualBudgetPlan: z.enum(
    ["no_budget_plan", "has_budget_plan", "complete_budget_plan"],
    {
  ...
  landOwnership: z.enum(
    ["no_contract", "rented_facilities", "rented_clear_terms", "owned_land"],
    {
...
export const schoolInfrastructureSchema = z.object({
  ...
  smartClassroom: z.enum(
    [
      "no_smart_classroom",
      "few_smart_classroom",
      "some_smart_classroom",
      "most_smart_classroom",
      "all_smart_classroom",
    ],
    {
  ...
  computerLabSize: z.enum(
    [
      "no_lab_size",
      "lab_size_one_door",
      "lab_size_two_doors",
      "lab_size_two_doors_windows",
    ],
    {
  ...
  // Workshops Section
  workshops: z.enum(
    ["no_workshops", "some_workshops", "most_workshops", "all_workshops"],
    {
  ...
  workshopElectricity: z.enum(
    ["no_workshop_electricity", "workshop_electricity"],
    {
  ...
  // Toilets Section
  staffToilets: z.enum(
    ["no_staff_toilets", "shared_staff_toilets", "sufficient_staff_toilets"],
    {
      required_error: "Staff toilets assessment is required",
    }
  ),
  learnerToilets: z.enum(
    [
      "no_learner_toilets",
      "shared_learner_toilets",
      "some_learner_toilets",
      "sufficient_learner_toilets",
    ],
    {
  ...
  internetConnectivity: z.enum(
    ["no_internet", "admin_internet", "full_internet"],
    {
      required_error: "Internet connectivity assessment is required",
    }
  ),
  adminFacilities: z.enum(
    ["no_admin_facilities", "some_admin_facilities", "full_admin_facilities"],
    {
  ...
  drinkingWater: z.enum(
    ["no_drinking_water", "some_drinking_water", "sufficient_drinking_water"],
    {
      required_error: "Drinking water assessment is required",
    }
  ),
  handWashing: z.enum(
    ["no_hand_washing", "some_hand_washing", "sufficient_hand_washing"],
    {
      required_error: "Hand washing facilities assessment is required",
    }
  ),
  cookingSpace: z.enum(
    ["no_cooking_space", "some_cooking_space", "sufficient_cooking_space"],
    {
      required_error: "Cooking space assessment is required",
    }
  ),
  playground: z.enum(
    ["no_playground", "some_playground", "sufficient_playground"],
    {
      required_error: "Playground assessment is required",
    }
  ),
  sportsFacilities: z.enum(
    [
       
      "no_sports_facilities",
      "some_sports_facilities",
      "sufficient_sports_facilities",
    ],
    {
  ...
  fireSafety: z.enum(
    ["no_fire_safety", "some_fire_safety", "sufficient_fire_safety"],
    {
  ...
  wasteManagement: z.enum(
    [
      "no_waste_management",
      "some_waste_management",
      "sufficient_waste_management",
    ],
    {
...
export const teachingResourcesSchema = z.object({
  ...
  materialsToolsEquipment: z.enum(
    ["no_tools", "few_tools", "some_tools", "most_tools", "all_tools"],
    {
  ...
  readingMaterials: z.enum(
    [
      "no_reading_materials",
      "few_reading_materials",
      "some_reading_materials",
      "most_reading_materials",
      "all_reading_materials",
    ],
    {
  ...
  // Furniture Section
  desks: z.enum(
    ["no_desks", "few_desks", "shared_desks", "most_desks", "all_desks"],
    {
      required_error: "Desk assessment is required",
    }
  ),
  teacherChairs: z.enum(
    [
      "no_teacher_chair",
      "few_teacher_chairs",
      "partial_teacher_chairs",
      "most_teacher_chairs",
      "all_teacher_chairs",
    ],
    {
  ...
  teacherTables: z.enum(
    [
      "no_teacher_table",
      "few_teacher_tables",
      "partial_teacher_tables",
      "most_teacher_tables",
      "all_teacher_tables",
    ],
    {
  ...
  sufficientTeachers: z.enum(
    ["insufficient", "most_teachers", "all_teachers"],
    {
      required_error: "Teacher sufficiency assessment is required",
    }
  ),
  qualifiedTeachers: z.enum(
    [
      "no_qualified_teachers",
      "few_qualified_teachers",
      "some_qualified_teachers",
      "most_qualified_teachers",
      "all_qualified_teachers",
    ],
    {
      required_error: "Teacher qualification assessment is required",
    }
  ),
  adminStaffQualification: z.enum(
    [
      "irrelevant_qualification",
      "quarter_relevant",
      "half_relevant",
      "three_quarter_relevant",
      "all_relevant",
    ],
    {
...
export type SchoolInfrastructureFormData = z.infer<
  typeof schoolInfrastructureSchema
>;
